...
int main(int argc, char **argv)
{
 ...
 char* ORIGINAL_grid = (char*) malloc(GLOBAL_nlines*GLOBAL_ncolumns * sizeof(char));

 // initialize the seed grid (GLOBAL_grid_A) depending on the chosen pattern
 gridInit(GLOBAL_grid_A,pattern);
 memcpy(ORIGINAL_grid, GLOBAL_grid_A,GLOBAL_nlines*GLOBAL_ncolumns * sizeof(char));
 ...
 //while(1)
 while((niters != NITERS) && (gridCompare(GLOBAL_grid_B, ORIGINAL_grid)==1))
 {
	 niters++;
	 sleep(1);
	 gridShow(GLOBAL_grid_A);
	 gridEvolve(GLOBAL_grid_A,GLOBAL_grid_B);
	 if(gridCompare(GLOBAL_grid_B,ORIGINAL_grid)==1)
		 memcpy(GLOBAL_grid_A, GLOBAL_grid_B,GLOBAL_nlines*GLOBAL_ncolumns * sizeof(char));
 }
 printf("\n");
 if (niters == NITERS) {
